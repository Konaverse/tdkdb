...
export default function SceneProjects({ projects }: SceneProjectsProps) {
  ...
  const mobileCardItems: iCardItem[] = projects.map((project) => ({
    title: project.title,
    description: `${project.type} — ${project.location}`,
    tag: project.status === 'completed' ? 'COMPLETED' : 'IN DEVELOPMENT',
    src: cloudinaryUrl(project.heroImageId, { width: 800 }),
    link: `/en/projects/${project.slug.current}`,
  ...
  return (
    <>
      {/* ── Mobile Layout ── */}
      <div className="block bg-void lg:hidden">
        <div className="py-20 text-center">
          <p className="text-label tracking-[0.3em] text-stone">THE WORK</p>
        </div>
        <CardsParallax items={mobileCardItems} />
      </div>

      {/* ── Desktop Layout ── */}
      <div
        ref={containerRef}
        className="hidden lg:block lg:h-[var(--desktop-height)]"
        style={{ '--desktop-height': containerHeight } as React.CSSProperties}
      >
        <section ref={sectionRef} className="relative h-screen w-full bg-void lg:overflow-hidden">
          {/* ── Section label ── */}
          <div
            ref={headingRef}
            className="pointer-events-none absolute left-6 top-10 z-20 hidden select-none lg:fixed lg:left-12 lg:top-10 lg:block"
          >
            <p className="text-label tracking-[0.3em] text-stone">THE WORK</p>
          </div>
          ...
          <div
            ref={stripRef}
            className="flex w-full flex-col will-change-transform lg:h-full lg:w-[var(--desktop-width)] lg:flex-row"
            style={{ '--desktop-width': `${count * 100}vw` } as React.CSSProperties}
          >
            {projects.map((project, i) => (
              <div
                key={project._id}
                className="relative h-screen w-full flex-shrink-0 overflow-hidden lg:h-full lg:w-screen"
              >
