...
// Derive a poster still (first frame) from a Cloudinary video URL.
function videoPoster(src: string) {
  return src.replace('/upload/', '/upload/so_0,w_800,q_auto,f_auto/').replace(/\.mp4$/, '.jpg');
}

// Background video that only decodes/plays while on-screen — three always-on
// loops were the main scroll-jank source, so we gate each one.
function BentoVideo({ src, className }: { src: string; className?: string }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        // Only decode/play once the card is meaningfully on-screen, so we don't
        // run all three loops at once while scrolling through the 180vh section.
...
function CraftTile({ Icon }: { Icon: typeof Compass }) {
  // Flat tile (no backdrop-filter): dozens of these animate over video, and a
  // moving backdrop-blur was the single most expensive thing in the scene.
  return (
    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.06] md:h-16 md:w-16">
...
function SectionLabel({
  ...
  return (
    ...
    >
      ...
      <span className="text-label uppercase text-white/70">{children}</span>
      <Sparkle strokeWidth={1.5} className="h-3 w-3 text-white/70" />
    </div>
  );
}
    ...
    >
      ...
      <div className="mb-10 flex flex-col items-start justify-between gap-8 md:mb-16 md:flex-row md:items-center lg:mb-24">
        <div className="max-w-3xl">
          <h2 className="font-sans text-display-md text-paper">
            We&apos;re TDK Design &amp; Build.
          </h2>
          <p className="mt-6 max-w-3xl text-body text-white/60 md:mt-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5 lg:h-[130vh] lg:grid-cols-3">
              ...
              <div className="grid grid-cols-[auto_auto_1fr] items-center gap-x-3 gap-y-3">
                {TIMELINE.map(([year, role, place]) => (
                  <div key={year} className="contents">
                    <span className="font-mono text-mono text-white/85">{year}</span>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-rows-[auto_1fr] md:gap-5">
          ...
          >
            ...
            <p className="mt-4 font-sans text-body leading-relaxed text-white/85">
            ...
            <p className="mt-4 font-mono text-mono text-white/70">
          ...
          <div className="relative flex min-h-[260px] flex-col overflow-hidden rounded-2xl bg-black p-5 md:p-6">
            ...
            <div className="relative z-10 flex flex-1 flex-col items-center justify-center">
              <span
                className="text-display-lg drop-shadow"
                style={{ color: 'var(--color-threshold)' }}
              >
                120+
              </span>
              <span className="mt-3 text-label uppercase text-white/85">Homes delivered</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-rows-[1fr_auto] md:gap-5">
          ...
          >
            ...
            <div className="mt-4 flex flex-col gap-1.5 font-mono text-mono text-white/90">
