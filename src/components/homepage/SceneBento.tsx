...
function CraftTile({ Icon }: { Icon: typeof Compass }) {
  return (
    <div className="liquid-glass h-14 w-14 shrink-0 rounded-xl md:h-16 md:w-16 flex items-center justify-center">
      <Icon strokeWidth={1.5} className="h-5 w-5 text-white/85 md:h-6 md:w-6" />
    </div>
  );
}

function MarqueeRow({ icons, direction }: { icons: typeof CRAFT_ROW_1; direction: 'left' | 'right' }) {
...
function SectionLabel({ children, align = 'center' }: { children: React.ReactNode; align?: 'center' | 'start' }) {
  return (
    <div
...
export default function SceneBento() {
  return (
    <section
      className="relative w-full bg-void text-paper antialiased px-4 py-12 sm:px-6 sm:py-16 md:px-10 md:py-24 lg:min-h-[180vh] lg:px-14 lg:py-32 flex flex-col justify-center"
      style={{ backgroundColor: 'var(--color-void)' }}
    >
      {/* ─── Header row ──────────────────────────────────────── */}
      <div className="mb-10 flex flex-col items-start justify-between gap-8 md:mb-16 lg:mb-24 md:flex-row md:items-center">
        <div className="max-w-3xl">
          ...
          <p
            className="mt-6 max-w-3xl text-sm leading-[1.6] text-white/60 md:mt-8 md:text-[15px]"
          >
      ...
      {/* ─── Bento grid ──────────────────────────────────────── */}
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5 lg:grid-cols-3 lg:h-[130vh]">
        {/* ── Column 1 — Track Record ─────────────────────── */}
        <div className="relative overflow-hidden rounded-2xl bg-black p-5 md:p-6 min-h-[420px] lg:min-h-0 flex flex-col">
          <video
        ...
        {/* ── Column 2 ─────────────────────────────────────── */}
        <div className="grid grid-cols-1 gap-4 md:gap-5 md:grid-rows-[auto_1fr]">
          {/* Client Voice */}
          <div
            className="relative overflow-hidden rounded-2xl p-5 md:p-6 noise-overlay"
          ...
          >
            ...
            >
              ...
              every finish quiet. The process felt graceful, and the result speaks for itself.&rdquo;
            </p>
            <p className="mt-4 text-[12px] text-white/70" style={{ fontFamily: 'var(--font-mono)' }}>
          ...
          {/* Stat */}
          <div className="relative overflow-hidden rounded-2xl bg-black p-5 md:p-6 min-h-[260px] flex flex-col">
            <video
        ...
        {/* ── Column 3 ─────────────────────────────────────── */}
        <div className="grid grid-cols-1 gap-4 md:gap-5 md:grid-rows-[1fr_auto]">
          {/* Craft / Disciplines */}
          <div className="relative overflow-hidden rounded-2xl bg-black p-5 md:p-6 min-h-[300px] flex flex-col">
            <video
          ...
          >
            ...
            <div
              ...
              style={{ fontFamily: 'var(--font-mono)' }}
            >
              <a href="mailto:hello@tdkdb.com" className="hover:text-white transition">
                hello@tdkdb.com
              </a>
              <a href="tel:+35722000000" className="text-white/70 hover:text-white transition">
                +357 22 000 000
              </a>
            </div>
          </div>
